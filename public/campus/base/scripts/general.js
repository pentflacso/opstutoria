...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function toggleDevMode(enabled) {
    ...
    if (!stylesLink) {
      console.error(
        'El elemento <link id="pent-styles"> no existe en la página.'
      );
      return;
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (body.classList.contains("pagelayout-course")) {
    ...
    allSections.forEach((section) => {
      ...
      const divider_number = [
        ...document.querySelectorAll("li.section-divider"),
      ].reverse();
    ...
    if (breadcrumb) {
      const firstBreadcrumbItem = breadcrumb.querySelector(
  ...
  function stickyFooter(breadcrumb) {
    const text_btn_new_entry = document.querySelector(".base-libreta")
      ? "Agregar nota"
      : "Escribir";
    ...
    if (breadcrumb) {
      ...
      if (firstItem) {
        ...
        breadcrumbTitle =
          firstItem.getAttribute("title") || firstItem.textContent.trim();
      }
    }
    ...
    const mode =
      urlParams.get("mode") == "single" || urlParams.get("rid")
        ? "single"
        : "list";
    ...
    if (stickyFooter) {
      ...
      navItems.forEach((item) => {
        ...
        if (item.classList.length === 1 && item.classList.contains("navitem")) {
          ...
          if (!document.body.classList.contains("bd-sv-pagination-none"))
            return;
          if (document.body.classList.contains("editing")) return;
    ...
    cards.forEach((card) => {
      ...
      if (matchClass) {
        const contentCard = document.querySelector(
          `.content-card-bd[data-collect="${matchClass}"]`
        );
  ...
  function accordionWhatsAppBd() {
    ...
    cards.forEach((card) => {
      ...
      if (matchClass) {
        const contentCard = document.querySelector(
          `.whatsapp-component[data-collect="${matchClass}"]`
        );
  ...
  function newEntries() {
    ...
    const emptyMessage = document.querySelector(
      'div[data-region="empty-message"]'
    );
    if (!emptyMessage) return;

    // Buscar formularios dentro de los divs con clase "singlebutton"
    const singleButtons = document.querySelectorAll(
      'div[data-region="empty-message"] .singlebutton form'
    );
  ...
  function reviewQuiz() {
    ...
    // Verificar si existe el botón de finalizar el cuestionario
    const finishReview = document.querySelector(
  ...
  function quizInputCheck() {
    ...
    const submitButton = form.querySelector(
      ".submitbtns input#mod_quiz-next-nav"
    );
  ...
  function verificarCheckboxes(checkboxes, submitButton, form) {
    ...
    divInfo.className =
      "alert alert-danger alert-block fade in alert-dismissible alert-checkbox";
    divInfo.textContent =
